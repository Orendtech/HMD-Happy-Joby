...
// ... (Previous functions: initializeUser, getUserProfile, etc. remain unchanged)
...
// ... (Gamification & CheckIn logic remains unchanged) ...
...
// NEW: Update Opportunity (Edit/Delete)
export const updateOpportunity = async (
    ...
    // 1. Handle New Structure (Visits -> Interactions -> Single Pipeline)
    if (location.visitIdx !== undefined && location.interactionIdx !== undefined) {
        if (report.visits && report.visits[location.visitIdx]) {
            ...
            if (visit.interactions && visit.interactions[location.interactionIdx]) {
                ...
                if (updatedData === null) {
                    delete interaction.pipeline; // Delete
                } else {
                    interaction.pipeline = updatedData; // Update
                    targetId = updatedData.id;
                }
            }
        }
    }
    // 2. Handle Legacy Structure in Visits (Visits -> Pipeline Array)
    ...
    // 3. Handle Root Legacy (Report -> Pipeline Array)
    ...
    // Save changes to Attendance Report
    await updateDoc(attRef, { report });
    
    // Sync with User Profile Active Pipeline
    if (targetId) {
        const userRef = getUserRef(userId);
        ...
        if (userSnap.exists()) {
            ...
            if (updatedData === null) {
                 // Delete: Remove from active memory
                 if (idx !== -1) activePipeline.splice(idx, 1);
            } else {
                 // Update: Sync data
                 if (idx !== -1) activePipeline[idx] = updatedData;
                 // Note: We don't push new here because this function is for editing existing reports
            }
