...
import { getTodayAttendance, getReminders, addReminder, addInteractionByAi, finalizeCheckoutByAi, createContactByAi, getGlobalPipelineForAi, addManagementLogByAi } from '../services/dbService';
...
export const LiveAIOverlay: React.FC<Props> = ({ user, userProfile }) => {
    ...
    // Immediate reaction to isAiEnabled from prop
    if (!userProfile || userProfile.isAiEnabled !== true) {
        return null;
    }
    ...
    const startSession = async () => {
        ...
        try {
            // Request permission explicitly before connection
            ...
            const systemInstruction = `
                ...
                กฎการทำงาน: บันทึกรายงานการเข้าพบ (add_interaction), แจ้งเตือน (add_reminder), เช็คเอาท์ (finalize_checkout)
                บุคลิก: สุขุม มืออาชีพ
                ${isPrivileged ? "ฟีเจอร์ลับ: ปลดล็อกรายงานบริหาร" : ""}
            `;
            ...
            // Explicitly resume to ensure Browser doesn't block
            ...
            const sessionPromise = ai.live.connect({
                ...
                callbacks: {
                    ...
                    onerror: (e: any) => { 
                        ...
                        setErrorMsg("Connection Error. Please check API Key."); 
                        stopSession(); 
                    },
                    onclose: () => { stopSession(); }
                }
            ...
            setErrorMsg(error.message || "Failed to start microphone."); 
            setIsConnecting(false); 
        }
    };
    ...
    return (
        <>
            <div className={`fixed z-[1000] cursor-grab active:cursor-grabbing transition-transform ${isDragging ? '' : 'duration-300'} group`} style={{ left: position.x, top: position.y }} onMouseDown={(e) => { setIsDragging(true); const clientX = e.clientX; const clientY = e.clientY; dragOffset.current = { x: clientX - position.x, y: clientY - position.y }; }}>
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[999] bg-[#020617] flex flex-col animate-fade-in text-white h-[100dvh] w-full overflow-hidden">
                    ...
                    <div className="relative z-10 w-full" style={{ paddingTop: 'env(safe-area-inset-top)' }}><div className="max-w-2xl mx-auto flex justify-between items-center h-24 px-8 opacity-60"><div className="flex items-center gap-3"><Radio size={16} className="text-cyan-400 animate-pulse" /><div className="flex flex-col"><span className="text-[10px] font-black uppercase tracking-[0.4em]">Neural Link Status</span><span className="text-[8px] font-bold text-cyan-500 uppercase tracking-widest">Auth: {getUserRoleLabel(userProfile)}</span></div></div><button onClick={toggleAI} className="p-4 bg-white/5 hover:bg-rose-500/20 rounded-3xl transition-all border border-white/5"><X size={24} /></button></div></div>
