  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] dark:bg-black flex flex-col items-center justify-center relative overflow-hidden transition-colors duration-700">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] opacity-20 dark:opacity-30 pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-orange-500 rounded-full blur-[120px] animate-pulse"></div>
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-amber-500 rounded-full blur-[120px] animate-pulse delay-1000"></div>
      </div>
        ...
        <div className="flex flex-wrap justify-center items-center gap-x-2 sm:gap-x-4">
          ...
          <div className="flex">
            {"Joby".split("").map((char, i) => (
              <span key={i} className="text-5xl sm:text-7xl font-black tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-orange-500 to-amber-600 letter-anim glow-text" style={{ animationDelay: `${(i + 5) * 0.1}s` }}>{char}</span>
            ))}
          </div>
        </div>
      ...
      <div className="absolute bottom-16 flex flex-col items-center gap-4 animate-enter opacity-0" style={{ animationDelay: '1s' }}>
        <div className="w-48 h-[2px] bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden relative">
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-orange-500 to-transparent w-full h-full animate-[glow-line_2s_infinite]"></div>
        </div>
