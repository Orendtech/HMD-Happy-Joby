  ...
  return (
    <div className="min-h-screen bg-[#F5F5F7] dark:bg-[#020617] flex flex-col items-center justify-center relative overflow-hidden transition-colors duration-700">
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="mt-8 flex items-center gap-3 px-5 py-2.5 bg-white/40 dark:bg-white/5 backdrop-blur-xl rounded-full border border-slate-200 dark:border-white/10 shadow-lg animate-enter opacity-0" style={{ animationDelay: '1.2s' }}>
  ...
  if (user && userProfile && userProfile.isApproved === false) {
      return (
        <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-white p-6 text-center space-y-4">
            ...
            <p className="text-gray-400">กรุณาติดต่อ Admin เพื่อเปิดใช้งาน</p>
            <button onClick={() => auth.signOut()} className="mt-4 px-6 py-2 bg-white/10 rounded-lg">กลับสู่หน้าล็อกอิน</button>
        </div>
      );
  }
    ...
    <HashRouter>
      <Routes>
        ...
        <Route path="/" element={user ? <Layout user={user} userProfile={userProfile} /> : <Navigate to="/login" />}>
           ...
           <Route path="ai" element={<AIAssistant user={user!} userProfile={userProfile} />} />
